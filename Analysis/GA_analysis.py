...
def rgb_filter(gaia_data_or_path, min_bp_rp=0.5, max_bp_rp=2, max_app_mag=19, max_abs_mag=5.2):
    """
    ...
    Parameters:
        ...
        min_bp_rp (float, optional): Minimum BP-RP color index (default=0.5).
        max_bp_rp (float, optional): Maximum BP-RP color index (default=2).
        max_app_mag (float, optional): Maximum apparent magnitude in dereddened G-band (default=19).
        max_abs_mag (float, optional): Maximum absolute magnitude in dereddened G-band (default=5.2).
    ...
    if isinstance(gaia_data_or_path, str) and gaia_data_or_path.endswith(".fits"):
        ...
        filtered_df, counts = _apply_rgb_filter(df, min_bp_rp, max_bp_rp, max_app_mag, max_abs_mag)
    ...
    # --------------- Handle Pandas DataFrame Input ---------------
        ...
        filtered_df, _ = _apply_rgb_filter(gaia_data_or_path, min_bp_rp, max_bp_rp, max_app_mag, max_abs_mag)
        return filtered_df
...
def _apply_rgb_filter(batch_data, min_bp_rp, max_bp_rp, max_app_mag, max_abs_mag):
    """
    ...
    # Colour index filter
    mask_bp_rp = (bp_rp > min_bp_rp) & (bp_rp < max_bp_rp)
...
def _compute_galpy_orbital_parameters(df):
    ...
    for i in tqdm(range(len(df)), desc="Processing stars", unit="star"):
        try:
            ...
            # Compute and store the key tidal stream parameters
            df.at[i, "energy"] = orbit.E(pot=MWPotential2014)
            df.at[i, "Lz"] = orbit.Lz()
            df.at[i, "R_gal"] = orbit.R()
            df.at[i, "Jz"] = orbit.Jz(pot=MWPotential2014)
...
def rgb_filter_batched(gaia_data_or_path, min_bp_rp=0.5, max_bp_rp=2, max_app_mag=19, max_abs_mag=5.2, batch_size=100000):
    """
    Filters Gaia data to select Red Giant Branch (RGB) stars in the based on 
    the BP-RP color index, apparent magnitude, and absolute magnitude.
    ...
    Parameters:
        ...
        max_app_mag (float, optional): Maximum apparent magnitude in dereddened G-band (default=19).
        max_abs_mag (float, optional): Maximum absolute magnitude in dereddened G-band (default=5.2).
        batch_size (int, optional): Number of rows to process at a time when using a FITS file (default=100000).

    Returns:
        pd.DataFrame or None: 
            - If a DataFrame is provided, returns the filtered DataFrame.
            - If a FITS file is provided, writes the filtered data to a new FITS file (`_halo_filtered.fits`).
    """
    ...
    if isinstance(gaia_data_or_path, str) and gaia_data_or_path.endswith(".fits"):
        ...
        # Open FITS file in memory-mapped mode
        with fits.open(input_fits, memmap=True) as hdul:
            # Get total number of rows
            total_rows = hdul[1].header['NAXIS2']  
            print(f"total_rows {total_rows}")
            # Get column names
            col_names = [col.name for col in hdul[1].columns]  

        # Check required columns exist
        required_columns = {'dered_BP_RP', 'dered_G', 'M_G'}
        missing_columns = required_columns - set(col_names)
        if missing_columns:
            raise ValueError(f"Missing required columns in FITS file: {missing_columns}")

        # Initialise output FITS file with same structure
        with fits.open(input_fits) as hdul:
            primary_hdu = fits.PrimaryHDU()
            table_hdu = fits.BinTableHDU.from_columns(hdul[1].columns)
            hdul_new = fits.HDUList([primary_hdu, table_hdu])
            hdul_new.writeto(output_fits, overwrite=True)
        ...
        with fits.open(input_fits, memmap=True) as hdul:
            for i in tqdm(range(num_batches), desc="Filtering Halo RGB Stars", unit="batch"):
                start = i * batch_size
                end = min((i + 1) * batch_size, total_rows)

                # Read batch data
                batch_data = hdul[1].data[start:end]

                # Apply filtering
                filtered_batch, counts = _apply_rgb_filter(batch_data, min_bp_rp, max_bp_rp, max_app_mag, max_abs_mag)
                ...
                # Append filtered batch to output FITS file
                if len(filtered_batch) > 0:
                    with fits.open(output_fits, mode='append') as hdul_out:
                        table_hdu = fits.BinTableHDU.from_columns(hdul[1].columns, nrows=len(filtered_batch))
                        table_hdu.data = filtered_batch 
                        hdul_out.append(table_hdu)

        # Print final summary
        print('how did i get here')
        print(f"\nTotal stars before filtering: {total_stars}")
        print(f"Stars passing BP-RP color filter: {total_bp_rp} ({total_bp_rp/total_stars:.2%})")
        print(f"Stars passing apparent magnitude filter: {total_app_mag} ({total_app_mag/total_stars:.2%})") 
        print(f"Stars passing absolute magnitude filter: {total_abs_mag} ({total_abs_mag/total_stars:.2%})")
        print(f"Stars passing all filters: {total_passed} ({total_passed/total_stars:.2%})")
        print(f"Halo RGB filtered FITS file saved as: {output_fits}")
...
#     fig, ax = plt.subplots(figsize=(8, 6), dpi=200)
#     plt.hist(hist.ravel(), bins=density_bins, color='gray', alpha=0.7, label="Density Distribution", edgecolor="black", rwidth = 1)
#     plt.yscale('log')
...
#     cut_hist = np.where(hist < vmin, 0, np.clip(hist, vmin, vmax))
#     im = ax.imshow(cut_hist.T, origin="lower", extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]],
...
#     # Faintest stars → Red
#     r_channel = normalize_channel(densities[2].T)  
#     # Medium brightness → Green
#     g_channel = normalize_channel(densities[1].T)  
#     # Brightest stars → Blue
#     b_channel = normalize_channel(densities[0].T)  
...
#     and returns their Galactic coordinates.
...
#     --------
#     selected_bins : list
#         Indices of bins that satisfy the star count condition.
#     selected_coords : list of tuples
#         List of Galactic coordinates (l, b) corresponding to selected bins.
#     """
...
#     # Convert to a list of (l, b) coordinate pairs
#     selected_coords = list(zip(selected_coords[0], selected_coords[1]))

#     return selected_bins, selected_coords
...
#     ### HTYUIKJCFGHJNB
#     clipped_density_map = np.where(density_map < vmin, 0)
#     clipped_density_map = np.where(density_map > vmax, 0)
#     if log_scale: 
...
#     # Faintest stars -- Red
#     r_channel = normalize_channel(density_maps_rgb[2]) 
#     # Medium brightness -- Green 
#     g_channel = normalize_channel(density_maps_rgb[1])
#     # Brightest stars -- Blue
#     b_channel = normalize_channel(density_maps_rgb[0]) 


#     # Ensure all channels have the same shape
#     assert r_channel.shape == g_channel.shape == b_channel.shape, "RGB channels must have the same shape"
#     print(f"RGB Channel Shape: {r_channel.shape}")
#     # Stack into RGB image
#     rgb_map = r_channel + g_channel + b_channel
#     print(len(rgb_map))
#     colors = np.vstack([r_channel, g_channel, b_channel]).T

#     # --------- Plot False-Color RGB Composite ---------
#     fig = plt.figure(figsize=(10, 6), dpi=200)
#     if log_scale: 
#         r_channel = np.log(r_channel +1)
#     hp.mollview(r_channel, cmap='Reds', title="Red (faint stars)", unit="Star Density")
#     hp.graticule(color="white", alpha=0.5)
#     plt.show()

#     fig = plt.figure(figsize=(10, 6), dpi=200)
#     if log_scale: 
#         g_channel = np.log(g_channel +1)
#     hp.mollview(g_channel, cmap='Greens', title="Green (Medium brightness stars)", unit="Star Density")
#     hp.graticule(color="white", alpha=0.5)
#     plt.show()

#     fig = plt.figure(figsize=(10, 6), dpi=200)
#     if log_scale: 
#         b_channel = np.log(_channel +1)
#     hp.mollview(b_channel, cmap='Blues', title="Blue (Brightest stars)", unit="Star Density")
#     hp.graticule(color="white", alpha=0.5)
#     plt.show()
